...
from SciAnalysis.analyses.Protocol import run_default
...
@delayed(pure=True)
class MasterMask:
    def __init__(self, master=None, origin=None):
        ''' MasterMask object.
        '''
        mask = master
        if mask is None and shape is None:
...
class Calibration:
    ...
    def __init__(self, attributes=None, detector='pilatus300'):
        ...
        if attributes is not None and detector is not None:
            ...
            calib_dict = load_calib_dict(attributes, self.calib_keymap,
                    self.calib_defaults).compute()('calib_dict').get()
            self.calibration = load_from_calib_dict(calib_dict, detector,
                    self.calib_defaults)('calibration')
        else:
            calibration = None
        ...
        print("rmap computation: {}".format(self.qx_map.key))
        self.r_map = _generate_r_map(self.calibration)
        self.q_map = _generate_q_map(self.calibration, self.qx_map, self.qy_map, self.qz_map)
    ...
    def set_keymap(self, name):
        if name == "cms":
            ...
            defaults = dict(ChainMap(Singlet('wavelength', None, 'Angstrom'),
                                Singlet('detectors', ['pilatus300'], 'N/A'), 
                                Singlet('beamx0', None, 'pixel'), 
                                Singlet('beamy0', None, 'pixel'), 
                                Singlet('sample_det_distance', None, 'm'), 
                                Singlet('pixel_size_x', None, 'pixel'),
...
def load_calib_dict(attributes, calib_keymap, calib_defaults):
    ...
    for key, newkey in calib_keymap.items():
        try:
            ...
            raise KeyError("There is an entry missing in header : {}. Cannot proceed".format(newkey))
...
def load_from_calib_dict(calib_dict, detector, calib_defaults):
    ...
    detector_key = detectors2D[detector]['image_key']['value']
    ...
    calib_tmp.update(Singlet('pixel_size_x', pixel_size_x_val, pixel_size_x_unit))
    calib_tmp.update(Singlet('pixel_size_y', pixel_size_y_val, pixel_size_y_unit))
    ...
    the detector face.'''
    c = (calib_tmp['pixel_size_x']['value']/1e6)/calib_tmp['sample_det_distance']['value']
    twotheta = np.arctan(c) # radians
    calib_tmp.update(Singlet('q_per_pixel', 2.0*calib_tmp['k']['value']*np.sin(twotheta/2.0),
                "1/Angstrom"))
...
def _generate_qxyz_maps(calibration):
    ...
    qy_map_data = kvector_mag*( np.cos(theta_f)*np.cos(alpha_f) - 1 ) # TODO: Check sign
