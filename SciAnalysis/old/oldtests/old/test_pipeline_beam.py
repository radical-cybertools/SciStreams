...
# TODO : Should make this a conform Reader in Beam
def load_from_uid(uid, db, key):
    ''' Load from uid
    
    '''
    return db.get_images(db[uid], key)[0]

def circular_average(image=None, calibration=None):
    #x0, y0 = calibration.x0, calibration.y0
    from skbeam.core.accumulators.binned_statistic import RadialBinnedStatistic
    result = SciResult()
    result['sq'] = np.ones((100))
...
def test_circavg():
    cmsdb_data = databases['cms']['data']
    cmsdb_analysis = databases['cms']['analysis']
    detector_key = 'pilatus300_image'
    # I randomly chose some header
    uids = [
            '89e8caf6-8059-43ff-9a9e-4bf461ee95b5',
            ]


    input_uids = beam.Create([uid])
    #transoform uids to data
    tran_uids_data = beam.Map(load_from_uid, db=cmsdb_data, key=detector_key)
    tran_circavg = beam.Map(circular_average, calibration=calib)

    p = beam.Pipeline()
    (p | input_uids | tran_uids_data | tran_circavg)
    p.run()
